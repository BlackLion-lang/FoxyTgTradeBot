...
import { getBalance, getTokenBalance } from '../../services/solana';
...
// Removed getBuyAmount and getSellPercent - now always prompts for input

// Bundle Buy Handler - Uses all balance of each wallet
...
// Bundle Sell Handler - Sells 100% of each wallet's token balance
export const handleBundleSell = async (
    ...
    // Execute directly with 100% sell - no input needed
    await executeBundleSellAllBalance(UserModel, userId, chatId, messageId, tokenAddress, bundleWallets);
};

// Execute bundle buy using ALL balance of each wallet
const executeBundleBuyAllBalance = async (
    ...
    // Get balances for each wallet (using all available balance minus rent-exempt and fees)
    ...
    for (const bundleWallet of bundleWallets) {
        try {
            ...
            // Calculate available balance (subtract rent-exempt minimum and transaction fees)
            ...
            // Only include wallets with meaningful balance (at least 0.001 SOL available)
    ...
    for (let i = 0; i < walletBalances.length; i++) {
        ...
        try {
            // Each wallet buys using ALL its available balance
            const result = await swapToken(
                userId,
                bundleWallet.publicKey,
                tokenAddress,
                availableBalance, // Use all available balance
                'buy',
                slippage,
                fee * 10 ** 11,
                0
...
// Execute bundle sell - sells 100% of each wallet's token balance
